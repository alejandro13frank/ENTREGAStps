...
int Per_buscarID(Persona* array[], int size, int valorBuscado, int* posicion)                    //cambiar orquesta
{
    int retorno=-1;
    int i;
    if(array!= NULL && size>=0)
        ...
        for(i=0;i<size;i++)
        {
            if(array[i]->id==id)
            {
                ...
                switch(opcionMenu)
                    ...
                    case 1:
                    utn_getName("\nIngrese nombre:","\nERROR",1,40,2,auxNombre);
                    Per_setNombre(array,auxNombre);
                    break;

                    case 2:
                    utn_getName("\nIngrese nombre:","\nERROR",1,40,2,auxApellido);
                    Per_getApellido(array,auxApellido);
                    break;

                    case 3:
                    utn_getSignedInt("\nIngrese edad:","\nERROR",1,4,1,99,2,&auxEdad);
                    Per_getEdad(array,auxEdad);
                    break;
