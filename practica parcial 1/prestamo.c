...
int prestamo_buscarInt(Prestamo array[], int size, int valorBuscado, int* posicion)                    //cambiar prestamo
{
    ...
    if(array!= NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                continue;
            else if(array[i].codigoDeLibro==valorBuscado)                                                   //cambiar campo codigoDeLibro
            {
                retorno=0;
                *posicion=i;
                break;
...
*
*/
int prestamo_alta(Prestamo array[],Socio arraySocio[],Libro arrayLibro[], int size,int sizeLibro,int sizeSocio, int* contadorID)                          //cambiar prestamo
{
    int retorno=-1;
    int posicion;
    int idLibro;
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        else
        {
            ...
            libro_listar(arrayLibro,sizeLibro);
            utn_getUnsignedInt("getUnsignedInt\n: ","\nError",1,6,1,&idLibro);
            if (libro_buscarID(arrayLibro,sizeLibro,idLibro,&posicionLibro)==-1)
            {
                printf("\nNo se encontro el ID ingresado");
            }else{
                    socio_listar(arraySocio,sizeSocio);
                    utn_getUnsignedInt("getUnsignedInt\n: ","\nError",1,6,1,&idSocio);
                    if (socio_buscarID(arraySocio,sizeSocio,idSocio,&posicionSocio)==-1)
                    {
                        printf("\nNo se encontro el ID ingresado");
                    }else{
    ...
    if(array!=NULL && sizeArray>0)
    {
        for(i=0;i<sizeArray;i++)
        {
            if(array[i].idUnico==valorBuscado)                                                        //cambiar si no se busca por ID
            {
                ...
                array[i].codigoDeLibro=0;                                                               //cambiar campo codigoDeLibro
...
int prestamo_ordenarPorString(Prestamo array[],int size)                              //cambiar prestamo
{
    ...
    if(array!=NULL && size>=0)
    {
        for (i = 1; i < size; i++)
        {
            ...
            bufferInt=array[i].codigoDeLibro;                                //cambiar campo codigoDeLibro
            ...
            j = i - 1;
            while ((j >= 0) && strcmp(bufferString,array[j].varString)<0)         //cambiar campo varString                 //Si tiene mas de un criterio se lo agrego, Ej. bufferInt<array[j].codigoDeLibro
            {                                                                                                               //buffer < campo ascendente   buffer > campo descendente
                ...
                array[j + 1].codigoDeLibro=array[j].codigoDeLibro;                        //cambiar campo codigoDeLibro
            ...
            array[j + 1].codigoDeLibro=bufferInt;                                                        //cambiar campo codigoDeLibro
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n codigoDeLibro: %d",
                       array[i].idUnico,array[i].codigoDeLibro);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
